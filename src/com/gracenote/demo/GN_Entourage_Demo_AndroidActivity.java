...
public class GN_Entourage_Demo_AndroidActivity extends Activity implements IGnAcrDelegate, IGnAudioSourceDelegate
{
	...
	private Button 						btnACR 			= null;
	private TextView 					txtStatus 		= null;
	private TextView 					txtResult 		= null;
	private boolean 					isListening 	= false;
	private GnUser						acrUser 		= null;
	private GnAcrAudioConfig 			acrAudioConfig	= null;
	private GnAcr						acr				= null;
	private GnAudioSourceMic			audioSource		= null;
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			// Initialize sdk. License enables both video and music.
    ...
    private void startACR() {
		...
		try {
			...
			updateStatusScreenInMainThread("ACR Version: " + GnSdk.getProductVersion(),true);
			updateResultScreenInMainThread("", true);
			...
			// ACR setup complete
	        // Results and status will come via delegate callback (acrResultReady and acrStatusReady respectively)
			
			btnACR.setText("Stop Identifying");
    ...
    private void stopACR() {
		...
		btnACR.setText("Start Identifying");
	}
	...
	private GnUser loadOrCreateUser(String clientId, String clientTag) throws GnException {
		...
		if (serializedUser == null) {
			joe = new GnUser(clientId, clientTag, "1.0",
					GnUserRegistrationType.newUser);
			...
			while (matches.hasNext()) {
				...
				if (airing != null) {
					...
					String displayString = String.format(
							...
							(subTitle != null ? subTitle.getDisplay() : "N/A"), channel.getCallsign(),
							matchPosition, ++nMatch);
